...
`scripts/weekly_retrain.py` — offline retraining pipeline for BPR and SAR.
...
**First run** — detected when neither `BPR_Recommender` nor `SAR_Recommender` exists in the MLflow Model Registry. Establishes the baseline, trains both models, registers them, and writes the manifest.
...
| `data/sar_interactions_sample.parquet` | Yes (first run) | Historical SAR training data. Columns: `user_id` (Int64), `book_id` (Int64), `weight` (Float32), `timestamp` (Int64) |
...
Override data paths with `--bpr-data-path` and `--sar-data-path` if your files live elsewhere.
...
# Custom data paths and lower threshold:
python scripts/weekly_retrain.py \
    --bpr-data-path /path/to/bpr_interactions_merged.parquet \
    --sar-data-path /path/to/sar_interactions_sample.parquet \
    --threshold 0.05
```
...
| `--sar-data-path` | `data/sar_interactions_sample.parquet` | Historical SAR parquet |
...
│           PASS → register new versions → Production
│                  write updated manifest, append run log
│                  exit 0
│           FAIL → register new versions → Archived
...
New app interactions are exported with **exactly the same column names and types** as the historical training parquets:
...
After the merge, the resulting parquet is passed directly to `train_bpr.main()` / `train_sar.main()`, which run the **identical** `preprocess_data()` functions on the entire combined dataset:

- Filter users with fewer than 5 interactions
- Deduplicate user-book pairs (BPR: keep max weight; SAR: keep most recent timestamp — matching each model's own `preprocess_data` policy)
- Cast weight to Float32

No separate pre-processing is applied to new rows. Everything is handled in one unified pass by the same code the training scripts already use.
...
| MLflow run (BPR) | MLflow experiment `BPR_Recommendations` | Params, metrics, artifacts |
| MLflow run (SAR) | MLflow experiment `SAR_Recommendations` | Params, metrics, artifacts |
| MLflow model version (BPR) | Registry: `BPR_Recommender` | New version promoted to `Production` |
| MLflow model version (SAR) | Registry: `SAR_Recommender` | New version promoted to `Production` |
| Merged retrain parquets | `data/bpr_interactions_retrain.parquet` `data/sar_interactions_retrain.parquet` | Intermediate files — safe to delete after training |

### After a rejected run

Same MLflow runs and model registry entries are created, but versions are transitioned to `Archived` instead of `Production`. The manifest is **not** updated — the previous accepted baseline remains in effect. The script exits with code 1.
...
```jsonc
{
  ...
  "sar_training_row_count": 1287000,
  "bpr_mlflow_run_id": "abc123...",
  "sar_mlflow_run_id": "def456...",
  "bpr_model_version": "2",
  "sar_model_version": "2",
  "bpr_baseline_metrics": { "ndcg_at_10": 0.104, ... }, // gate for next run's promotion
  "sar_baseline_metrics": { "ndcg_at_10": 0.073, ... },
  "outcome": "accepted",
