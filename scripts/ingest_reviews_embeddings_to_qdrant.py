...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ingest review embeddings from Parquet into Qdrant with streaming reads "
            "and batched parallel uploads. Stores book_id in payload for reranking."
        )
    )
    parser.add_argument(
        "--input-path",
        default="data/reviews_base_embeddings.parquet",
        help="Input parquet path with review_id/review_text/embedding/book_id columns.",
    )
    parser.add_argument(
        "--collection-name",
        choices=[name.value for name in CollectionNames],
        default=CollectionNames.REVIEWS.value,
    ...
    parser.add_argument(
        "--book-id-column",
        default="book_id",
        help="Column used as payload.book_id (required for reranking).",
    )
    parser.add_argument(
        "--vector-size",
        type=int,
...
def _validate_args(args: argparse.Namespace, input_path: Path) -> None:
    ...
    if not str(args.book_id_column).strip():
        raise ValueError("--book-id-column cannot be empty")
...
def _build_points(
    ...
    book_ids = batch_frame.get_column(book_id_column).to_list()

    for row_id, embedding, book_id in zip(ids, embeddings, book_ids):
        try:
        ...
        # book_id is required for reranking
        if book_id is None:
            id_conversion_errors += 1
            continue

        # Coerce book_id to int if possible
        try:
...
def main() -> None:
    ...
    logger.info("Starting Qdrant ingestion for reviews")
    ...
    logger.info("Columns: id=%s, embedding=%s, book_id=%s",
                args.id_column, args.embedding_column, args.book_id_column)
    logger.info(
    ...
    for record_batch in reader.iter_batches(
        columns=[args.id_column, args.embedding_column, args.book_id_column],
        ...
        batch_frame = pl.from_arrow(record_batch)
        # Note: embedding column may be Array (fixed-size) or List type.
        # For Array types, size is guaranteed by schema, so we only check not null.
        valid_frame = batch_frame.filter(
            pl.col(args.id_column).is_not_null()
            & pl.col(args.embedding_column).is_not_null()
            & pl.col(args.book_id_column).is_not_null()
        )
