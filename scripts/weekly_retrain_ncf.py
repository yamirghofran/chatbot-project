#!/usr/bin/env python
"""
Weekly offline retraining pipeline for the NCF recommendation model.

FIRST RUN: detected automatically when NCF_Recommender does not exist in the
...
Usage:
    python scripts/weekly_retrain_ncf.py [options]

    # dry-run (no DB, no writes):
...
import scripts.train_ncf as _ncf
...
logger = logging.getLogger("weekly_retrain_ncf")
...
_DEFAULT_MANIFEST_PATH = _project_root / "data" / "retrain_manifest_ncf.json"
_DEFAULT_RUN_LOG_PATH = _project_root / "data" / "retrain_runs_ncf.jsonl"
_SAMPLE_ARTIFACTS_DIR = _project_root / "sample-artifacts"

_NCF_REGISTRY_NAME = "NCF_Recommender"

# Merged parquet path written during a retrain run (overwritten each time).
_MERGED_NCF_PATH = _project_root / "data" / "ncf_interactions_retrain.parquet"
...
def _load_ncf_baseline_metrics() -> dict:
    """
    Load the NCF baseline evaluation metrics from sample-artifacts.
    These were produced by the original training run.
    """
    path = _SAMPLE_ARTIFACTS_DIR / "ncf" / "evaluation_metrics.json"
    if not path.exists():
        logger.warning(f"NCF sample metrics not found at {path}; using empty baseline")
        return {}
    with open(path) as f:
        metrics = json.load(f)
    logger.info(f"Loaded NCF baseline metrics from {path}")
    return metrics

# ---------------------------------------------------------------------------
# Dataset size counter — reuses train_ncf's preprocessing exactly
# ---------------------------------------------------------------------------

def _count_ncf_preprocessed_rows(data_path: str) -> int:
    """
    Load and preprocess the NCF parquet using the exact same pipeline as
    train_ncf.main() to get the post-preprocessing row count.
    Also logs user counts before and after the min-interaction filter.
    """
    cfg = _ncf.DEFAULT_CONFIG
...
def _merge_ncf(historical_path: str, new_interactions: Optional[pl.DataFrame]) -> Path:
    """
    Merge the historical NCF parquet with new app interactions.

    Produces data/ncf_interactions_retrain.parquet with columns:
    ...
    remaining compatible with NCF's internal string-to-int mapping.
    The file is then passed to train_ncf.main() which applies the full
    preprocessing pipeline (user filter ≥5, dedup, Float32 cast) uniformly
    across all rows.
    """
    cfg = _ncf.DEFAULT_CONFIG
    historical = _ncf.load_data(
        historical_path,
        required_columns=[cfg["col_user"], cfg["col_item"]],
    )
    ...
    if new_interactions is not None and new_interactions.height > 0:
        app_ncf = new_interactions.select(["user_id", "book_id", "weight"])
        merged = pl.concat([historical, app_ncf], how="diagonal_relaxed")
    else:
        merged = historical
    ...
    merged.write_parquet(_MERGED_NCF_PATH)
    logger.info(f"Wrote merged NCF dataset ({merged.height} rows) → {_MERGED_NCF_PATH}")
    return _MERGED_NCF_PATH
...
def _run_first_train(args: argparse.Namespace, client: MlflowClient) -> int:
    ...
    """
    ncf_path = Path(args.ncf_data_path)
    if not ncf_path.exists():
        logger.error(
            f"NCF interactions file not found: {ncf_path}\n"
            "This file must be created before running the NCF retraining pipeline.\n"
            "Run the NCF data preparation step first to produce this parquet."
        )
        return 1

    logger.info("=" * 60)
    logger.info("FIRST RUN detected — NCF_Recommender not found in MLflow")
    logger.info(f"  NCF data : {args.ncf_data_path}")
    logger.info("=" * 60)

    ncf_baseline = _load_ncf_baseline_metrics()  # noqa: F841 — stored in manifest
    ...
    ncf_historical_rows = _count_ncf_preprocessed_rows(args.ncf_data_path)
    logger.info(f"  NCF historical rows (post-preprocessing): {ncf_historical_rows:,}")
    ...
    logger.info("Merging NCF dataset …")
    merged_ncf_path = _merge_ncf(args.ncf_data_path, new_interactions)

    # Train — delegate entirely to the existing training script.
    ncf_config = _ncf.DEFAULT_CONFIG.copy()
    ncf_config["data_path"] = str(merged_ncf_path)
    logger.info("Training NCF …")
    _, ncf_recs_path, ncf_metrics, ncf_run_id = _ncf.main(ncf_config)
    logger.info(f"NCF training complete (run_id={ncf_run_id})")
    logger.info(f"  NCF metrics: { {k: round(v, 4) for k, v in ncf_metrics.items()} }")

    # Register — on first run we always promote to Production.
    ncf_version = _register_model(
        client, ncf_run_id, "ncf_model", _NCF_REGISTRY_NAME, "Production", args.dry_run
    )
    ...
    manifest = {
        ...
        "ncf_training_row_count": ncf_historical_rows,
        "ncf_mlflow_run_id": ncf_run_id,
        "ncf_model_version": ncf_version,
        "ncf_baseline_metrics": ncf_metrics,
        "outcome": "accepted",
        "ncf_recs_path": str(ncf_recs_path),
    }
...
def _run_retrain(args: argparse.Namespace, client: MlflowClient) -> int:
    ...
    logger.info(f"NCF data path                : {args.ncf_data_path}")
    ...
    # Count new deduplicated NCF rows using the same preprocessing pipeline.
    ...
    if new_interactions is not None and new_interactions.height > 0:
        ...
        new_ncf_rows = tmp.height
    logger.info(f"New deduplicated NCF rows: {new_ncf_rows:,}")

    previous_row_count = manifest.get("ncf_training_row_count", 0)
    threshold_ratio = new_ncf_rows / max(previous_row_count, 1)
    logger.info(
        f"Threshold check: {new_ncf_rows:,} new / {previous_row_count:,} previous = "
    ...
    accepted = _metrics_acceptable(
        ncf_metrics, manifest.get("ncf_baseline_metrics", {}), args.tolerance
    )
    logger.info(f"Metrics gate: NCF={'PASS' if accepted else 'FAIL'} → {'ACCEPTED' if accepted else 'REJECTED'}")

    ncf_stage = "Production" if accepted else "Archived"
    ncf_version = _register_model(
        client, ncf_run_id, "ncf_model", _NCF_REGISTRY_NAME, ncf_stage, args.dry_run
    )
    ...
    if accepted:
        new_manifest = {
            **run_record,
            "last_accepted_train_at": now_iso,
            "ncf_training_row_count": new_ncf_rows + previous_row_count,
            "ncf_baseline_metrics": ncf_metrics,
            "ncf_recs_path": str(ncf_recs_path),
        }
        if not args.dry_run:
...
def _print_summary(record: dict) -> None:
    ...
    if "ncf_recs_path" in record:
        logger.info(f"NCF recommendations → {record['ncf_recs_path']}")
        logger.info("  Update NCF_PARQUET_URL in your backend .env to this path.")
    if "ncf_model_version" in record and record["ncf_model_version"]:
        logger.info(f"NCF model version  : {record['ncf_model_version']}")
    logger.info(sep)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Offline weekly retraining pipeline for the NCF recommendation model"
    )
    parser.add_argument(
        "--ncf-data-path",
        default=str(_project_root / "data" / "ncf_interactions_merged.parquet"),
        help="Path to the historical NCF interactions parquet",
    )
    parser.add_argument(
        "--manifest-path",
...
def main() -> int:
    ...
    is_first_run = not _model_registered(client, _NCF_REGISTRY_NAME)
    logger.info(f"NCF_Recommender registered: {not is_first_run} → {'first run' if is_first_run else 'retrain'}")
...
if __name__ == "__main__":
    try:
        ...
        logger.error("Unhandled exception in weekly_retrain_ncf.py")
